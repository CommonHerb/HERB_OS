...
**Last updated:** February 20, 2026 (Session 56 -- Sweep the Small Items + Legend Text)
...
2. **Phase 1 Migration Inventory — Behavioral Logic Still in C** -- Session 51 audited every WHAT decision remaining in `kernel_main.c`. These 14 items are the roadmap for migrating policy from C to HERB. Each item is a place where C makes a behavioral decision that could instead be expressed as HERB tensions or HERB state. ~~2 items~~ **SOLVED (Session 52):** priority cycling and program assignment. ~~2 items~~ **SOLVED (Session 53):** keystroke mapping and text parsing. ~~2 items~~ **SOLVED (Session 54):** mode routing and click routing. ~~3 items~~ **SOLVED (Session 55):** color mapping, terminated limit, display order (implicit). ~~4 items~~ **SOLVED (Session 56):** timer interval, buffer capacity, shell protection value, legend/help text. **13 of 14 solved, 1 remaining (shell action interpretation — mostly mechanism, help text sub-item migrateable).**
   ...
   **Command/Shell (3 items — 2 SOLVED Session 53):**
   ...
   - **Shell action interpretation:** C interprets ShellCtl.action codes — 10→swap, 20→list, 30→help (`handle_shell_action()` ~L2659). HERB tensions set the code; C decides what each code means and performs the action. **Session 56 analysis:** Mostly MECHANISM — load (1-4) delegates to cmd_spawn/HERB spawn policy, swap (10) loads binaries (hardware), spawn (40) delegates to cmd_spawn, unknown (-1) logs error. Only two sub-items are policy: help text (action=30, string content) and list container set (action=20, which containers to iterate). Help text is migrateable using the same Legend pattern (string entities). List containers are lower priority (debugging view with compile-time container name constants).
