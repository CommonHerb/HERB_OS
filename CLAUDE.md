...
- `boot/test_bare_metal.py` — Automated QEMU test harness (127 tests)
- `boot/Makefile` — Build system (make run → QEMU graphics, make run-text → VGA text, make test-bare-metal → automated tests)
- `boot/herb_hw.asm` — Phase 2 assembly: 16 hardware functions (port I/O + privileged CPU ops + serial port + PIC/PIT), MS x64 ABI callable
- `boot/herb_os.img` — 108KB bootable disk image (graphics), 86KB (text)
...
### Tests
- 748 total: 127 bare metal, 449 v2 runtime, 142 v1 runtime, 17 API, 13 cross-format
